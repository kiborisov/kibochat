...
weight_decay = 0.0
init_lr_frac = 0.02
checkpoint_every = 100 # how often to save intermediate checkpoints (-1 = disable)
...
model_config_kwargs = dict(model.config.__dict__)
...
def save_sft_checkpoint(step_label, reason):
    if not master_process:
        return
    base_dir = get_base_dir()
    depth = model.config.n_layer
    model_tag_local = f"d{depth}"
    checkpoint_dir = os.path.join(base_dir, "chatsft_checkpoints", model_tag_local)
    metadata = {
        "step": step_label,
        "val_loss": val_loss,
        **latest_metrics,
    ...
    prefix = "✅ " if reason == "final" else ""
    print(f"{prefix}Saved model checkpoint to {checkpoint_dir} (step {step_label:05d}, {reason})")
...
for step in range(num_iterations):
    ...
    if checkpoint_every > 0 and completed_steps % checkpoint_every == 0:
        save_sft_checkpoint(completed_steps, "periodic")

# Save the model at the end of the run
total_steps = num_iterations
save_sft_checkpoint(total_steps, "final")
