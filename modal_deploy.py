"""
Deploy kibochat on Modal: Full LLM Training & Inference Pipeline

Complete training pipeline on Modal's cloud GPUs - from tokenizer to deployed chat interface.
...
- Also works with H100s (faster)

Setup:
    1. Install Modal: pip install modal && modal setup

    2. Optional - Set up secrets for WandB:
       modal secret create kibochat-secrets WANDB_API_KEY=your_key HUGGINGFACE_TOKEN=your_token

Usage:
    modal run modal_deploy.py                                    # Full pipeline
    modal run modal_deploy.py::main --num-data-shards=8 --depth=12  # Quick test
    modal run modal_deploy.py::chat_cli --source=sft --prompt="Hello"
    modal run modal_deploy.py::chat_web                          # Web UI
"""
...
from modal.image import Image as ModalImage
...
BASE_DIR = "/checkpoints/.cache/nanochat"
...
NANOCHAT_IMAGE = (
    ...
    .run_commands("rm -rf /root/nanochat/.venv")
    .run_commands(
        "bash -c 'source $HOME/.cargo/env && uv sync --extra gpu && uv run maturin develop --release --manifest-path rustbpe/Cargo.toml'"
    )
    .env(
        {
            "OMP_NUM_THREADS": "1",
            "NANOCHAT_BASE_DIR": "/checkpoints/.cache/nanochat",
            "HF_HOME": "/data/.cache/huggingface",
            "VIRTUAL_ENV": "/root/nanochat/.venv",
            "CARGO_HOME": "/root/.cargo",
            "RUSTUP_HOME": "/root/.rustup",
            "PATH": "/root/nanochat/.venv/bin:/root/.cargo/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
            "PYTHONPATH": "/root/nanochat/.venv/lib/python3.11/site-packages",
        }
    )
)
...
def sync_old_checkpoints():
    """Copy legacy checkpoints from /data to /checkpoints."""
    import os
    import shutil

    src = "/data/.cache/nanochat/"
    dst = "/checkpoints/.cache/nanochat/"
    os.makedirs(dst, exist_ok=True)
    shutil.copytree(src, dst, dirs_exist_ok=True)
    print(f"Synced checkpoints from {src} -> {dst}")
...
def ensure_eval_bundle():
    """Ensure the CORE eval bundle is present and not corrupted."""
    import os
    import shutil
    import subprocess
    import tempfile
    ...
    core_yaml_path = os.path.join(eval_bundle_path, "core.yaml")

    if os.path.exists(core_yaml_path):
        print(f"Eval bundle ready at {eval_bundle_path}")
        return eval_bundle_path

    if os.path.exists(eval_bundle_path):
        print("Eval bundle directory exists but is incomplete. Removing before re-download...")
        shutil.rmtree(eval_bundle_path)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        extracted_dir = os.path.join(tmpdir, "eval_bundle")
        shutil.move(extracted_dir, eval_bundle_path)

    print(f"Eval bundle downloaded to {eval_bundle_path}")
    data_volume.commit()
    return eval_bundle_path
...
def train_base_model(
    ...
    resume: bool = False,
    resume_step: int = -1,
    resume_model_tag: Optional[str] = None,
):
    """
    ...
    if model_tag:
        extra_args.append(f"--model_tag={model_tag}")
    if resume_model_tag:
        extra_args.append(f"--resume_model_tag={resume_model_tag}")
    if resume:
        extra_args.append("--resume=True")
    if resume_step >= 0:
        extra_args.append(f"--resume_step={resume_step}")
...
def evaluate_base_model(max_per_task: int = 500):
    ...
    extra_args = []
    if max_per_task > 0:
        extra_args.append(f"--max-per-task={max_per_task}")
...
@asgi_app()
def chat_web():
    """Serve the chat model via a Modal-hosted web UI."""
    import os

    from scripts.chat_web import ChatWebConfig, build_chat_web_app
    ...
    config = ChatWebConfig(
        num_gpus=env("CHAT_WEB_NUM_GPUS", NUM_GPUS_INFERENCE, int),
        source=env("CHAT_WEB_SOURCE", "sft", str),
        temperature=env("CHAT_WEB_TEMPERATURE", 0.6, float),
        top_k=env("CHAT_WEB_TOP_K", 50, int),
        max_tokens=env("CHAT_WEB_MAX_TOKENS", 512, int),
        model_tag=os.environ.get("CHAT_WEB_MODEL_TAG"),
        step=env("CHAT_WEB_STEP", None, lambda v: int(v) if v else None),
        dtype=os.environ.get("CHAT_WEB_DTYPE", "bfloat16"),
        device_type=os.environ.get("CHAT_WEB_DEVICE", "cuda"),
        host="0.0.0.0",
        port=8080,
    )
    return build_chat_web_app(config)
...
def main(
    ...
    base_checkpoint_every: int = 1000,
    mid_checkpoint_every: int = 250,
    sft_checkpoint_every: int = 100,
    wandb_run: str = "dummy",
    base_model_tag: str = "",
    base_resume: bool = False,
    base_resume_step: int = -1,
    base_resume_model_tag: str = "",
):
    """
    ...
    if run_base:
        ...
        train_base_model.remote(
            ...
            model_tag=base_model_tag or None,
            resume=base_resume,
            resume_step=base_resume_step,
            resume_model_tag=base_resume_model_tag or None,
        )
        if run_eval:
    ...
    if run_mid:
        ...
        train_mid_model.remote(
            device_batch_size=device_batch_size_base,
            wandb_run=wandb_run,
            checkpoint_every=mid_checkpoint_every,
        )
        if run_eval:
    ...
    if run_sft:
        ...
        train_sft_model.remote(
            ...
            wandb_run=wandb_run,
            source="mid",
            checkpoint_every=sft_checkpoint_every,
        )
        if run_eval:
